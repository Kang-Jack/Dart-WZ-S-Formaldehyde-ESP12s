...
void handle_WZ_S_LCD() {
    ...
    if (success)
    {
        if ((!AdminEnabled) && WifiEnabled)
        {
            if (DateTime.minute<3 && DateTime.second<2)
            {
                memset(r1_str_temp, 0, sizeof(r1_str_temp));
                memset(r2_str_temp, 0, sizeof(r2_str_temp));
                dtostrf(r1, 4, 1, r1_str_temp);
                dtostrf(r2, 1, 3, r2_str_temp);
                memset(playload, 0, sizeof(playload));
                snprintf(playload, sizeof(playload), "{ppb:%s,hoco:%s}", r1_str_temp,r2_str_temp);
                publish_msg(playload);
            } 
        } 
    }
}
